...
def load_all_players():
    ...
    file_path = "players_live.json"
    
    # Check if the dataset is present in the repository root directory
    if not os.path.exists(file_path):
        st.error("Missing 'players_live.json' file! Please ensure you uploaded it to the root of your GitHub repository.")
        return pd.DataFrame()
    ...
    try:
        ...
        st.error(f"Error reading local 'players_live.json' file: {e}")
        return pd.DataFrame()
...
if df.empty:
    st.info("Upload your downloaded 'players_live.json' file to the root of your GitHub repository to initialize the app panels.")
else:
